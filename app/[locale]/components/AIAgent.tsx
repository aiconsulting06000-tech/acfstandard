...
export default function AIAgent() {
  ...
  async function sendMessage(text: string) {
    ...
    try {
      ...
      setMessages(prev => [...prev, { text: data.text || data.error || "Error", type: "bot" }]);
    } catch {
      ...
      {open && (
        <div className="acf-agent-overlay">
          ...
          <div className="acf-agent-box">
            ...
            {/* Input */}
            <div style={{ padding: "12px 20px", borderTop: `1px solid ${C.bd1}`, display: "flex", gap: 8 }}>
              <input
                value={input}
                onChange={e => setInput(e.target.value)}
                onKeyDown={e => { if (e.key === "Enter") sendMessage(input); }}
                placeholder={t.placeholder}
                style={{ flex: 1, background: C.navy3, border: `1px solid ${C.bd1}`, borderRadius: 10, padding: "12px 16px", color: "#fff", fontSize: 14, outline: "none", fontFamily: "'Inter', sans-serif" }}
              />
              <button
                onClick={() => sendMessage(input)}
                style={{ background: `linear-gradient(135deg, ${C.gold}, ${C.gold2})`, border: "none", borderRadius: 10, width: 44, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}
              >
