...
LANGS = {
    ...
    "de": {
        ...
        "prod_intro": "Fünf komplementäre Produkte, die den ACF-Standard operationalisieren.",
        "products": [("ACF AI Act Checker","COMPLIANCE-TOOL","Kostenloses Diagnosetool zur Bewertung Ihrer Konformität mit dem EU AI Act.",["EU AI Act Compliance-Diagnose","Rollenbasierte Pflichtenzuordnung","Automatische Risikoklassifizierung"]),("ACF Score","DIAGNOSE-TOOL","Proprietärer Souveränitäts-Score, der Ihre Entscheidungsunabhängigkeit über 6 Governance-Dimensionen misst.",["Zusammengesetzter Souveränitäts-Score","6-Achsen-Radarvisualisierung","Personalisierter Aktionsplan pro Achse"]),("ACF Auditor","AUDIT-PLATTFORM","Geführte Audit-Plattform zur Bewertung der digitalen und agentischen Reife Ihrer Organisation über 7 gewichtete Dimensionen. Branchenkalibriertes Scoring, automatische Erkennung kritischer Muster und Generierung einer umsetzbaren Roadmap.",["Geführtes Audit 107 Fragen / 7 Dimensionen mit Branchenkalibrierung","Agentic Readiness Score + Souveränitäts-Score","Automatisch generierte 3-Phasen-Roadmap"]),("ACF Control","GOVERNANCE-PLATTFORM","Echtzeit-Governance-Dashboard, das Ihre 18 Souveränitäts-KPIs mit adaptivem Gating überwacht.",["18 KPIs über 6 Governance-Achsen","Adaptives Gating mit menschlicher Eskalation","Fälschungssichere Audit-Logs"]),("ACF Certification","UNABHÄNGIGE ZERTIFIZIERUNG","Unabhängige Zertifizierung, die die Konformität mit dem ACF-Governance-Standard bescheinigt.",["Level 1, 2 und 3 Pfad","Öffentlich verifizierbares Badge","Jährliche Erneuerung + kontinuierliches Monitoring"])],
    ...
    "pt": {
        ...
        "prod_intro": "Cinco produtos complementares que operacionalizam o padrão ACF.",
        "products": [("ACF AI Act Checker","FERRAMENTA DE CONFORMIDADE","Ferramenta de diagnóstico gratuita para avaliar sua conformidade com o EU AI Act.",["Diagnóstico de conformidade EU AI Act","Mapeamento de obrigações por papel","Classificação automática de risco"]),("ACF Score","FERRAMENTA DIAGNÓSTICA","Score de soberania proprietário que mede sua independência decisional em 6 dimensões de governança.",["Score composto de soberania","Visualização radar 6 eixos","Plano de ação personalizado por eixo"]),("ACF Auditor","PLATAFORMA DE AUDITORIA","Plataforma de auditoria guiada que avalia a maturidade digital e agêntica da sua organização em 7 dimensões ponderadas. Scoring calibrado por indústria, detecção automática de padrões críticos e geração de roadmap acionável.",["Auditoria guiada 107 perguntas / 7 dimensões com calibração industrial","Agentic Readiness Score + Score de Soberania","Roadmap de 3 fases gerado automaticamente"]),("ACF Control","PLATAFORMA DE GOVERNANÇA","Dashboard de governança em tempo real que monitora seus 18 KPIs de soberania com gating adaptativo.",["18 KPIs em 6 eixos de governança","Gating adaptativo com escalada humana","Logs de auditoria infalsificáveis"]),("ACF Certification","CERTIFICAÇÃO INDEPENDENTE","Certificação independente que atesta a conformidade ao padrão de governança ACF.",["Percurso Level 1, 2 e 3","Badge publicamente verificável","Renovação anual + monitoramento contínuo"])],
    ...
    "nl": {
        ...
        "prod_intro": "Vijf complementaire producten die de ACF-standaard operationaliseren.",
        "products": [("ACF AI Act Checker","COMPLIANCE TOOL","Gratis diagnosetool om uw naleving van de EU AI Act te beoordelen.",["EU AI Act compliance diagnose","Rolgebaseerde verplichtingsmapping","Automatische risicoclassificatie"]),("ACF Score","DIAGNOSTISCH INSTRUMENT","Eigen soevereiniteitsscore die uw beslissingsOnafhankelijkheid meet over 6 governance-dimensies.",["Samengestelde soevereiniteitsscore","6-as radarvisualisatie","Gepersonaliseerd actieplan per as"]),("ACF Auditor","AUDIT-PLATFORM","Begeleide auditplatform dat de digitale en agentische maturiteit van uw organisatie evalueert over 7 gewogen dimensies. Industriegekalibreerde scoring, automatische detectie van kritieke patronen en generatie van een uitvoerbare routekaart.",["Begeleide audit 107 vragen / 7 dimensies met industriekalibratie","Agentic Readiness Score + Soevereiniteitsscore","Automatisch gegenereerde 3-fasen routekaart"]),("ACF Control","GOVERNANCE-PLATFORM","Real-time governance dashboard dat uw 18 soevereiniteits-KPI's bewaakt met adaptieve gating.",["18 KPI's over 6 governance-assen","Adaptieve gating met menselijke escalatie","Onvervalsbare auditlogs"]),("ACF Certification","ONAFHANKELIJKE CERTIFICERING","Onafhankelijke certificering die conformiteit met de ACF-governance-standaard bevestigt.",["Level 1, 2 en 3 traject","Openbaar verifieerbaar badge","Jaarlijkse vernieuwing + continue monitoring"])],
    ...
    "ru": {
        ...
        "prod_num": "06", "prod_title": "ACF® Products",
        "prod_intro": "Five complementary products operationalizing the ACF standard.",
        "products": [("ACF AI Act Checker","COMPLIANCE TOOL","Free diagnostic tool to assess your compliance with the EU AI Act. Identify your obligations based on your role and risk level.",["EU AI Act compliance diagnostic","Role-based obligation mapping","Automated risk classification"]),("ACF Score","DIAGNOSTIC TOOL","Proprietary sovereignty score measuring your decisional independence across 6 governance dimensions.",["Composite sovereignty score","6-axis radar visualization","Personalized action plan per axis"]),("ACF Auditor","AUDIT PLATFORM","Guided audit platform evaluating the digital and agentic maturity of your organization across 7 weighted dimensions. Industry-calibrated scoring, automatic critical pattern detection and actionable roadmap generation.",["Guided audit 107 questions / 7 dimensions with industry calibration","Agentic Readiness Score + Sovereignty Score","Automatically generated 3-phase roadmap"]),("ACF Control","GOVERNANCE PLATFORM","Real-time governance dashboard monitoring your 18 sovereignty KPIs with adaptive gating.",["18 KPIs across 6 governance axes","Adaptive gating with human escalation","Tamper-proof audit logs"]),("ACF Certification","INDEPENDENT CERTIFICATION","Independent certification attesting compliance with the ACF governance standard.",["Level 1, 2 and 3 pathway","Publicly verifiable badge","Annual renewal + continuous monitoring"])],
        "about_num": "07", "about_title": "About",
...
def generate_pdf(lang_code, t):
    ...
    for code, title, desc in t["principles"]:
        ...
        c.setFillColor(WHITE); c.setFont("Helvetica-Bold", 10); c.drawString(92, y + 20, title)
    ...
    for code, title, desc in t["layers"]:
        ...
        c.setFillColor(WHITE); c.setFont("Helvetica-Bold", 9.5); c.drawString(92, y + 18, title)
        c.setFillColor(GREY); c.setFont("Helvetica", 8.5); c.drawString(62, y + 3, desc)
        y -= 48
    ...
    y -= 15; c.setFillColor(GREY); c.setFont("Helvetica", 11); c.drawString(40, y, t["prod_intro"])
    y -= 30
    df = 9; dlh = 12; ff = 8.5; flh = 12; mw = W - 130
    for name, label, desc, features in t["products"]:
        words = desc.split(); line = ""; dl = 0
        for word in words:
            test = line + " " + word if line else word
            if c.stringWidth(test, "Helvetica", df) < mw: line = test
            else: dl += 1; line = word
        if line: dl += 1
        bh = 38 + dl * dlh + 6 + len(features) * flh + 10
        c.setFillColor(DARK_CARD); c.roundRect(40, y - bh, W - 80, bh, 5, fill=1, stroke=0)
        c.setStrokeColor(GOLD); c.setLineWidth(1.5); c.line(50, y - 2, W - 50, y - 2)
        ly = y - 16; c.setFillColor(GOLD); c.setFont("Helvetica", 7.5); c.drawString(55, ly, label)
        ly -= 14; c.setFillColor(WHITE); c.setFont("Helvetica-Bold", 14); c.drawString(55, ly, name)
        ly -= 12; c.setFillColor(GREY); c.setFont("Helvetica", df)
        words = desc.split(); line = ""
        for word in words:
            test = line + " " + word if line else word
            if c.stringWidth(test, "Helvetica", df) < mw: line = test
            else: c.drawString(55, ly, line); ly -= dlh; line = word
        if line: c.drawString(55, ly, line); ly -= dlh
        ly -= 3
        for feat in features:
            c.setFillColor(GOLD); c.setFont("Helvetica", ff); c.drawString(60, ly, "\u2713")
            c.setFillColor(LIGHT_GREY); c.setFont("Helvetica", ff); c.drawString(76, ly, feat)
            ly -= flh
        y -= (bh + 6)
